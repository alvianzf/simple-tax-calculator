...
const ResultCard = ({ result, mode, inputTax, thrInput = 0, bonusInput = 0 }) => {
  ...
  const comparison = useMemo(() => {
    const getComparisonFunc = (taxAmount) => {
    ...
    return getComparisonFunc(taxForSarcasm);
  }, [taxForSarcasm]);

  // Persistent random comments for conditional remarks
  const noThrNoBonusComment = useMemo(() => NO_THR_NO_BONUS_COMMENTS[Math.floor(Math.random() * NO_THR_NO_BONUS_COMMENTS.length)], []);
  const noBonusComment = useMemo(() => NO_BONUS_COMMENTS[Math.floor(Math.random() * NO_BONUS_COMMENTS.length)], []);
  const noThrComment = useMemo(() => NO_THR_COMMENTS[Math.floor(Math.random() * NO_THR_COMMENTS.length)], []);
